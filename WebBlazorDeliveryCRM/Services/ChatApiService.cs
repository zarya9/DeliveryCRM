...
public class ChatApiService
{
    ...
    public async Task<(bool ok, string? path, string? error)> UploadChatAttachmentAsync(IBrowserFile file, int userId)
    {
        try
        {
            ...
            var response = await _http.PostAsync($"/api/Files/chat-attachment?userId={userId}", content);
